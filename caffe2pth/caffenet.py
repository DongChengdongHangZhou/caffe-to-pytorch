...
class CaffeNet(nn.Module):
    ...
    def load_weights(self, caffemodel):
        ...
        for l in layers:
            lmap[l.name] = l
#lmap[layers[1]['name']].blobs
#a.bias.data.copy_(torch.from_numpy(np.array(lmap[layers[1]['name']].blobs[1].data)))
        layers = self.net_info['layers']
        layer_num = len(layers)
        i = 0
        while i < layer_num:
        ...
        while i < 2:
            ...
            if ltype == 'Convolution':
                ...
                try:
                    ...
                    kernel_size = (int(convolution_param['kernel_h']),int(convolution_param['kernel_w'])) 
                ...
                try:
                    pad = convolution_param['pad']
                except:
                    pad = (int(convolution_param['pad_h']),int(convolution_param['pad_w'])) 
                ...
                blob_width[tname] = int(math.floor((blob_width[bname] + 2*int(convolution_param['pad_w']) - int(convolution_param['kernel_w'])/stride + 1)))
                blob_height[tname] = int(math.floor((blob_height[bname] + 2*int(convolution_param['pad_h']) - int(convolution_param['kernel_h']))/stride + 1))
                i = i + 1
            elif ltype == 'BatchNorm':
                momentum = 0.9
                ...
                shape = [1, blob_channels['data'], blob_height['data'], blob_width['data']]
                ...
                for idx, tn in enumerate(tname):
                    ...
                    blob_width[tn] = blob_width['data']
                    blob_height[tn] = blob_height['data']
